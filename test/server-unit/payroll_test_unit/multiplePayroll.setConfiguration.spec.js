...
describe('test/server-unit/payroll-test-unit/Multiple Payroll Config Controller', () => {
  ...
  it('should calculate seniority in years correctly', async () => {
    ...
    // Retrieve the call that inserts into the payment table
    const insertPaymentQuery = transactionStub.addQuery
      .getCalls()
      .find(call => call.args[0].includes('INSERT INTO payment'));
    ...
    // Calculate seniority separately since config does not return it
    const hiringDate = moment(req.body.data.employee.hiring_date);
    const periodDateTo = moment(req.body.data.periodDateTo);
    const yearsOfSeniority = periodDateTo.diff(hiringDate, 'years');

    // Assert that seniority is calculated correctly
    expect(yearsOfSeniority).to.equal(5); // 5 years difference between hiring date and period end

    // Sanity checks for payment data
    expect(paymentData).to.have.property('employee_uuid', 'emp-uuid'); // verify employee UUID
    expect(paymentData).to.have.property('basic_salary').that.is.a('number'); // verify basic salary exists

    // Ensure the HTTP response was sent correctly
    sinon.assert.calledOnce(res.sendStatus);
    sinon.assert.calledWith(res.sendStatus, 201);
  });
