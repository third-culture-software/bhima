...
describe(`Tests to verify that enabling "Payment by Index"
  ...
  const defaultEmployeesNumber = 2;
  ...
  it('should generate payroll payslip and verify that rubricsIndexes (otherProfits) contain elements', async () => {
    try {
      ...
      // we assume there should be 2 payslips for 2 employees
      expect(reportRes.body.dataEmployees.length).to.equal(defaultEmployeesNumber);
      ...
      reportRes.body.dataEmployees.forEach(employee => {
        ...
        expect(employee.otherProfits.length).to.be.greaterThan(0);

        employee.otherProfits.forEach(profit => {
          expect(profit).to.have.all.keys('label', 'value');
        });
