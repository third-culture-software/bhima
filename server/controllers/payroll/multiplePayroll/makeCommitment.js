...
async function config(req, res, next) {
  try {
    ...
    // RESOLVED (@lomamech): Enterprise settings (cost center mode & pension transaction type)
    // are now retrieved directly from the DB instead of the session to prevent
    // inconsistencies caused by outdated session data.
