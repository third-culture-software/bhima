...
3. Create a new branch called `release-next` (for example) that is based on the upstream repository to do your release with.  If one exists on upstream, remove it first.  For example, if the main BHIMA githup repository is called `upstream` in your local development setup, do:
...
12. Push your branch to the BHIMA upstream repository:
